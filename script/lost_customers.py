...
from services.customer_processing import (
    ...
    generate_json_data,
    convert_decimal  # imported but not used in current snippet
)
...
def compute_reference_date(today):
    ...
    If today is between the 1st and 14th, use the first day of the month.
    Otherwise, use the 14th (or 15th for non-February months).
    """
    if 1 <= today.day <= 14:
        return today.replace(day=1)
    else:
        # For February, use 14th; for other months use 15th.
...
def load_sales_data(engine, customer_ids, reference_date):
    ...
    """
    # Ensure customer_ids is a tuple for the query
...
def assign_campaign_types(customers, sales_data):
    ...
    """
    # Convert types for further operations
...
def main():
    ...
    # For category M3, extract customer_ids for sales query
    ...
    # Build final DataFrame to be inserted into the customer_campaigns table
